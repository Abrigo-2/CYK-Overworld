    ...
    if not Player.ishurting and (damage ~= nil or not onHitOverridden) then
        ...
        if not Encounter["doneFor"] then
            if (type(damage) == "string" and damage[1] ~= "-") or (type(damage) == "number" and damage >= 0) then
                ...
                --PlaySoundOnceThisFrame("hurtsound")  --not needed it seems...?
            end
        end
    end
end
...
function PrepareArenaForAnim(x, y, width, height, movePlayer, immediate)
    Arena.MoveToAndResize(Misc.cameraX + x, Misc.cameraY + y, width, height, movePlayer, immediate)
    Arena.inner.alpha = 0
    Arena.outer.alpha = 0
            ...
            elseif not bullet["noGraze"] then
                ...
                if grazed then
                    ...
                    if bullet["grazed"] then
                        local tpIntake =  bullet["TPGain"] and bullet["TPGain"] * 0.025 or 0.05
                        SuperCall(Encounter, "CYK.TP.Set", tpIntake, true)
                    else
