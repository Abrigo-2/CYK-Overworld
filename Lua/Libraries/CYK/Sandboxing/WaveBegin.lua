...
function CreateProjectileForReal(spritename, initial_x, initial_y, layer, isAbs)
    ...
    local projectile = (isAbs and _CreateProjectileAbs or _CreateProjectile)("Bullets/"..spritename, initial_x, initial_y, layer)
    if not layer then
...
-- literally just CreateSprite but the sprite gets added to a table so it can get destroyed at the end of the turn.
function CreateArenaSprite(spritename, layer)
    -- Starts the right CYF function
    local layer = layer or "Arena"
    local sprite = CreateSprite("Bullets/" .. spritename, layer)
    sprite.MoveToAbs(Arena.x, Arena.y + Arena.height/2)

    table.insert(Encounter["CYK"].ArenaSpritesPool, sprite)
    return sprite
end
