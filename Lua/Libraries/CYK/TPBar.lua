return function(CreateYourKris)
    ...
    self.maxValue  = 250                       -- Highest amount of TP the team can have. 
    self.barValue = 0                          -- TP value used to display the bar
    self.previewValue = 0                      -- TP value used to display the bar's preview (white bar)
    self.barSpeed = self.maxValue * 0.01       -- Speed at which the bar can change per frame
    self.previewSpeed = 8                      -- Speed at which the preview bar can change per frame
    ...
    function self.toggleBarAlpha(alpha)
        self.bar.bg.alpha = alpha
        self.bar.preview.alpha = alpha
        self.bar.previewDown.alpha = alpha
        self.bar.bar.alpha = alpha
        self.bar.previewTPLoss.alpha = alpha
        self.bar.mask.alpha = alpha
        self.staticText.alpha = alpha
        self.num1.alpha = alpha
        self.num2.alpha = alpha
    end
    self.toggleBarAlpha(0)
    ...
    function self.Update()
        ...
        if self.trueValue ~= self.previewValue then
            ...
            if self.previewValue >= self.maxValue then
                ...
                local associatedYScale = self.previewValue / self.maxValue * self.ySize
                if gt then
        ...
        if self.trueValue ~= self.barValue then
            ...
            -- Computes the scale of the bar and applies it
            local associatedYScale = self.barValue / self.maxValue * self.ySize
            if gt then
                self.bar.bar.yscale = associatedYScale
            else
                self.bar.previewDown.yscale = associatedYScale
            ...
            associatedYScale = self.trueValue / self.maxValue * self.ySize
    ...
    function self.PreviewTPLoss(loss)
        ...
        self.bar.previewTPLoss.yscale = loss / self.maxValue * self.ySize
        self.bar.previewTPLoss.y = self.barValue / self.maxValue * self.ySize
    end
    ...
    function self.Hide()
        self.bar.bg.absx = Misc.cameraX + 41
    ...
    function self.AnchorToCamera()
        self.bar.bg.absx = Misc.cameraX - 39
        self.bar.bg.absy = Misc.cameraY + 435

        self.staticText.absx = Misc.cameraX -80
        self.num1.absx = Misc.cameraX -60
        self.num2.absx = Misc.cameraX -52
