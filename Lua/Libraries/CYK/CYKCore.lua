return function ()
    ...
    self.quitting = CreateSprite("CreateYourKris/UI/Quitting/0", "Top")
    ...
    --self.Background = (require "Libraries/CYK/Blue-Stripes Background")(self)  --Pssst... Replace the above value with this one. It'll be fun...
    ...
    function self.Confirm(force)
        ...
        elseif self.state == "PLAYERTURN" then
            ...
            if ( (self.TxtMgr.text.allLinesComplete or self.turn == 0) and not self.stopPlayerTurnProgress and #self.currentPlayerActionQueue==0 ) or force then
                ...
                if self.SetTurn(1, true) == "after" then
                    -- But before that, reset player's animations to Idle.
                    for i = 1, #self.players do
                        if (self.players[i].sprite["currAnim"] ~= "Idle" and self.players[i].action == "" ) then
                    ...
                    if player.action == "Defend" or player.action == "Fight" or player.action == "" then
                        ...
                        -- Otherwise, set the Player's animation before any code is ran.
                        -- That way, you can overwrite it during the HandleSpell/Act functions.
                        local playerAnim = player.action == "Act" and "Spare" or player.action
                        ...
                        if player.action == "Act" then
                            if enemy.acts[player.subAction].requiredPlayers then
                                local availableActions = self.GetAvailableActions(enemy, true)
                        ...
                        elseif player.action == "Magic" and player.subAction ~= "" then
                            ProtectedCYKCall(player.PrepareCustomSpell, enemy, player.subAction)

                            -- Add the actual magic handling to the queue.
                        ...
                        elseif player.action == "Item" and player.subAction ~= ""  then
                            ...
                            -- Display the DIALOGUE for this item.
                            ProtectedCYKCall(HandleItemDialogue, player, targets, item)
                            
                            -- Add the actual item handling to the queue.
                            table.insert( self.currentPlayerActionQueue, player )
                        -- SPARE
    ...
    -- Actions performed in PLAYERTURN are added to a queue, then performed here once the current animation is over.
    self.currentPlayerActionQueue = {}
    function self.PerformQueuedPlayerActions(force)
        local force = force or false

        for i=#self.currentPlayerActionQueue, 1, -1 do
            local nextPlayer = self.currentPlayerActionQueue[i]
            
            local isActionFrame = self.anims[nextPlayer.sprite["anim"]][nextPlayer.sprite["currAnim"]][3].actionFrame == nextPlayer.sprite.currentframe
            if isActionFrame or force then
                if nextPlayer.action == "Magic" then
                    ProtectedCYKCall(nextPlayer.HandleCustomSpell, nextPlayer.target, nextPlayer.subAction)
                elseif nextPlayer.action == "Item" then
                    local itemData = self.Inventory.items[self.Inventory.inventory[nextPlayer.subAction]]
                    local targets = { }
                    -- Get the targets of the item
                    if string.match(itemData.targetType, "All") then
                        targets = string.match(itemData.targetType, "Player") and self.players or self.enemies
                    else
                        table.insert(targets, nextPlayer.target)
                    end
                    local item = self.Inventory.inventory[nextPlayer.subAction]
                    -- Use the item
                    self.Inventory.UseItem(nextPlayer.subAction)
                    ProtectedCYKCall(HandleItem, nextPlayer, targets, item, itemData)
                elseif nextPlayer.action == "Spare" then
    ...
    -- Triggers at the end of a normal encounter
    function self.EncunterWrapUp()
        if not self.TP.isBeingHidden then self.TP.Hide() end
        if not self.UI.isBeingHidden then self.UI.Hide() end
        self.TxtMgr.text.progressmode = "none"
        self.TxtMgr.text2.progressmode = "none"

        if not self.Background.isBeingHidden then 
            self.Background.Hide() end
        
        for i = 1, #self.allPlayers do
            if (i <= #Overworld.party) then
                Overworld.party[i].hp = self.allPlayers[i].hp
                Overworld.party[i].maxhp = self.allPlayers[i].maxhp
            end
        end

        for i=1, #self.UI.HPChangeTexts do
            for j=1, #self.UI.HPChangeTexts[i] do
                self.UI.HPChangeTexts[i][j].Remove()
            end
        end
        self.UI.HPChangeTexts = {}

        OWinventory = self.Inventory.inventory
        self.CleanEntites()
        self.cleanAnims()
        Audio.Stop()
        NewAudio.SetVolume("BGM", 0)
        NewAudio.Unpause("BGM")
    ...
    function self.State(newState, secondaryData)
        ...
        local isInBattle = newState ~= "NONE" and newState ~= "SETUP" and newState ~= "INTRO"
        if isInBattle then
        ...
        if oldState == "DEFENDING" then
            ...
            -- If we're trying to enter the state DEFENDING again, we need to go through the state ACTIONSELECT to reset the wave and choose a new one
        ...
        if oldState == "ATTACKING" then
            ...
            for i = #self.AtkMgr.attackingPlayers, 1, -1 do
                ...
                for j=1, #attackingPlayer.visor["trails"] do
                    attackingPlayer.visor["trails"][j].alpha = 0 end
        ...
        -- Creates all objects used up in the fight.
        if newState == "SETUP" then
            self.frame = 0
            self.SetEntitiesStart()
            Inventory.SetInventory( OWinventory )

            self.UI.BuildPlayerUI()
            self.TxtMgr.text.progressmode = "none"
            self.TxtMgr.text2.progressmode = "none"
        ...
        elseif newState == "ACTIONSELECT" then
            ...
            if self.turn > 0 then
                local player_buttons = self.players[self.turn].UI.buttons
                for i = 1, #player_buttons do
                    if player_buttons[i]["active"] then
                        self.choiceIndex = i
                        set = true
                    end
                end
        ...
        elseif newState == "DEFENDING" then
            ...
            OldState("DEFENDING")
            Wave[1].Call("PrepareArenaForAnim", { arenapos[1], arenapos[2], 
                        arenasize[1], arenasize[2], false, true })
        ...
        elseif newState == "BEFOREDONE" then
            ...
            for i = 1, #self.players do
                -- In case a player was Down by the end of the fight, heal them a bit.
                if self.players[i].hp <= 0 then
                    self.players[i].hp = math.ceil(self.players[i].maxhp/13)
    ...
    function self.MovePlayer()
        ...
        if self.TxtMgr.twoColumns or Player.sprite.absx ~= Misc.cameraX + 62 then
            Player.sprite.x = self.choiceIndex % 2 == 0 and 222 or (self.TxtMgr.lastTwoColumns and 22 or 62)
            --self.choiceIndex % 2 == 0 and 222 or (self.TxtMgr.lastTwoColumns and 22 or 62)
        end
    ...
    function self.ChangePlayerTurn(anim, prev)
        ...
        -- Only if we're going to the next Player, change the old Player's face UI and change their animation
        if not prev then
            if oldAction ~= "" then
                oldPlayer.UI.faceSprite.Set("CreateYourKris/Players/" .. oldPlayer.sprite["anim"] .. "/UI/" .. oldPlayer.action)
            end
            if self.state ~= "PLAYERTURN" then
                self.SetAnim(oldPlayer, anim)
            end
        end
        ...
        elseif setTurnResult == "before" then
            ...
            --self.EnableButton(self.turn, self.choiceIndex, true)
    ...
    function self.MoveCursor(val, isHorz)
        ...
        if self.TxtMgr.textDActive then
            ...
            local description_tab = ""
            if player.action == "Magic" then
                description_tab = self.spells[player.abilities[currentObject]].description
            elseif player.action == "Act" then
                description_tab = player.target.acts[self.GetAvailableActions(player.target, true).commands[currentObject]].description
            elseif player.action == "Defend" then
                description_tab = "[font:uidialog][novoice][instant][color:808080]" .. "\n[color:ff8040]" .. tostring(self.DefenderCost) .. "% TP"
            else
                description_tab = self.Inventory.items[self.Inventory.GetCurrentInventory().inventory[currentObject]].description
            end
            self.TxtMgr.textDescription.SetText({ description_tab })
    ...
    -- Ergo, resets the Player's buttons.
    function self.EnableButton(playerID, buttonIndex, doNotShow)
        local playerUI = self.players[playerID].UI
        for i = 1, 5 do
            local button_name = "CreateYourKris/UI/Buttons/" .. playerUI.buttonNames[i]

            if buttonIndex == i then
                if not doNotShow then
                    button_name = button_name .. "S" .. (CYK.CrateYourKris and "T" or "")
                end
                playerUI.buttons[i]["active"] = true
            else
                if not doNotShow then
                    button_name = button_name .. (CYK.CrateYourKris and "T" or "")
                end
                playerUI.buttons[i]["active"] = false
            end
              

            playerUI.buttons[i].Set( button_name )
    ...
    function self.Update()
        ...
        -- Player inputs
        local isInBattle = self.state ~= "NONE" and self.state ~= "SETUP" and self.state ~= "INTRO"
        if awaitingCYKInput and isInBattle then
        ...
        -- Execute the current Player's UpdateTurn() function if their turn has been stopped
        if self.state == "PLAYERTURN" then
        ...
        -- Frame count update
        if isInBattle then
            self.frame = self.frame + 1 end

        -- Updates the QUITTING text position
        self.quitting.absx = Misc.cameraX + 4
        self.quitting.absy = Misc.cameraY + 476
    end

    -- Updates the pointerly dot in the targetCursor of the players.
    function self.UpdateTargetCursor()
        if self.state == "NONE" then
            return
        end

        for i=1, #self.players do
            local targetParent = self.players[i].targetCursor
            if targetParent.alpha > 0 then
                -- WIP
                local new_x = targetParent["pointer"].x + (targetParent["pointer"]["speed"] * math.cos(targetParent["pointer"]["angle"]) )
                local new_y = targetParent["pointer"].y + (targetParent["pointer"]["speed"] * math.sin(targetParent["pointer"]["angle"]) )

                if (math.abs(new_y) > 36) or (math.abs(new_x) > 36) then
                    targetParent["pointer"]["angle"] = targetParent["pointer"]["angle"] + (math.pi/12)

                    new_x = targetParent["pointer"].x + (targetParent["pointer"]["speed"] * math.cos(targetParent["pointer"]["angle"]) )
                    new_y = targetParent["pointer"].y + (targetParent["pointer"]["speed"] * math.cos(targetParent["pointer"]["angle"]) )
                end

                targetParent["pointer"].MoveTo(new_x, new_y)

                targetParent["pointer"].alpha = 1 
            else
                targetParent["pointer"].alpha = 0
            end
        end
    end
    ...
    function self.UpdateQuitting()
        ...
        if hasEscapeChanged and self.escapeFrame < 58 then
            self.quitting.alpha = self.escapeFrame / 15
            self.quitting.Set("CreateYourKris/UI/Quitting/" .. tostring(math.floor(self.escapeFrame / 12)))
        end
    end
    ...
    function self.UpdateIntro()
        if self.state == "INTRO" then
            ...
            for i=1, #self.enemies do
                -- Moves the enemies into the arena during the Intro.
                if self.enemies[i].posX > Misc.cameraX + enemypositions[i][1] then
                    self.enemies[i].Move(-8.2, 0)
                    introDone = false
                -- Makes sure the enemies haven't gone past their place.
                else
                    self.enemies[i].MoveToAbs(Misc.cameraX + enemypositions[i][1], self.enemies[i].posY)
                end
            end

            for i = 1, #self.players do
                -- If all of the Players' Intro animation has ended, go to the ACTIONSELECT state
                if not (self.players[i].sprite["currAnim"] ~= "Intro" or self.players[i].sprite.animcomplete) then
                    introDone = false end
            end

            if introDone then self.State("ACTIONSELECT") end
        end
    end
