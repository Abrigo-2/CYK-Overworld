return function(CYK)
    ...
    -- SHINY!! SO SHINY!!!
    self.textShiny = CreateText({ "" }, {600, 200}, 500, "LowerUI")
    self.textShiny.SetParent(CYK.UI.hider)
    ...
    self.text2.x = 260
    self.text2.y = 78
    self.text2.deleteWhenFinished = false
    ...
    function self.SetText(text, text2, noReplaceTexts)
        if CYK.state == "NONE" then
            ...
            self.textShiny.progressmode = "manual"
        ...
        if not text2 then
            ...
            for i = 1, #text do
                ...
                text[i] = "[font:uidialog2][speed:1.5][voice:monsterfont-s]" .. textCommands .. "¤¤" .. string.gsub(string.gsub(textRealText, "\n", "\n¤¤"), "\r", "\n")
                shiny[i] = "[font:vertgradient][speed:1.5][novoice]" .. "¤¤" .. string.gsub(string.gsub(self.SilenceText(textRealText), "\n", "\n¤¤"), "\r", "\n")
                -- TODO: Detect size star and space for each \n and begin of text
                text2[i] = "[font:uidialog2][speed:1.5]" .. self.SilenceText(textCommands) .. "[novoice]*[charspacing:-37] [charspacing:1][alpha:00]" ..
    ...
    function self.ScanTextLine(nextLine)
        ...
        if self.textLine <= #self.lastText or not nextLine then
            ...
            if string.find(text, "SetFaceSprite") then
                ...
                self.textShiny.x = 180
                self.textShiny.textMaxWidth = 440
            else
                self.text.x = 80
                ...
                self.textShiny.x = 80
                self.textShiny.textMaxWidth = 540
            end
        end
        if doNextLine then
            self.text.NextLine()
