...
function self.Update()
    ...
    elseif self.textObject then
        ...
        elseif self.textProgress == 1 and self.animTimer == 390 then
            ...
            local destination = (self.choice == true and Misc.cameraX+241) or (self.choice == false and Misc.cameraX+394) or Misc.cameraX+320
        ...
        if Input.Confirm == 1 and self.textObject.lineComplete and ((self.textProgress == 1 and self.choice ~= nil) or self.textProgress ~= 1) and self.textProgress < 3 then
            ...
            if self.textProgress == 1 then
                ...
                self.textObject.MoveTo(Misc.cameraX+105, Misc.cameraY+294)

                -- Create new soul
                self.heart = CreateSprite("CreateYourKris/dr-heart", self.layer)
                self.heart.MoveTo(Misc.cameraX+320, Misc.cameraY+100)
                self.heart.alpha = 0

                -- Create "YES"
                self.yes = CreateText("[noskip][instant][font:uidialog]YES", { Misc.cameraX+221, Misc.cameraY+94 }, 40, self.layer)
                self.yes.color = { 1, 1, 1, 0 }
                self.yes.HideBubble()
                self.yes.progressmode = "none"

                -- Create "NO"
                self.no = CreateText("[noskip][instant][font:uidialog]NO", { Misc.cameraX+381, Misc.cameraY+94 }, 40, self.layer)
            ...
            elseif self.textProgress == 2 then
                ...
                -- "YES"
                if self.choice then
                    self.textObject.MoveTo(Misc.cameraX+124, Misc.cameraY+294)
                    self.textObject.SetText({self.text[3]})

                    self.textProgress = 3

                    self.cover2 = CreateSprite("UI/sq_white", self.layer)
                    self.cover2.MoveTo(Misc.cameraX+320, Misc.cameraY+240)
                    self.cover2.Scale(640 / 4, 480 / 4)
                    self.cover2.alpha = 0
                -- "NO"
                else
                    self.textObject.MoveTo(Misc.cameraX+144, Misc.cameraY+294)
...
function self.StartGameOver()
    if not self.active then
        ...
        self.cover.MoveTo(Misc.cameraX + 320, Misc.cameraY + 240)
...
function self.StartText()
    self.textObject = CreateText({self.text[1], self.text[2]}, {Misc.cameraX+163, Misc.cameraY+294}, 450, self.layer)
