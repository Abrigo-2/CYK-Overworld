return function(CYK)
    ...
    function self.relocateUI()
        self.lowerUIRoot.absx = Misc.cameraX + 0
        self.upperUIRoot.absx = Misc.cameraX + 0

        -- Put the UI out of the screen
        self.lowerUIRoot.absy = Misc.cameraY + 0
        self.upperUIRoot.absy = Misc.cameraY + 0

        -- Hides the bottom part of the screen
        self.hider.absx = Misc.cameraX + 0
        self.hider.absy = Misc.cameraY + 0

        -- UI bars. AESTHETIC
        self.bar1.absx = Misc.cameraX + 320
        self.bar1.absy = Misc.cameraY + 152

        self.bar2.absx = Misc.cameraX + 320
        self.bar2.absy = Misc.cameraY + 116
    end

    function self.toggleAlpha(alpha)
        self.lowerUIRoot.alpha = alpha
        self.hider.alpha = alpha
        self.bar1.alpha = alpha
        self.bar2.alpha = alpha
    end
    self.toggleAlpha(0)
    ...
    function self.UpdatePlayerHP(player)
        ...
        elseif player.hp <= math.ceil(player.maxhp / 3) then --4.2
    ...
    function self.BuildPlayerUI()
        ...
        for i = 1, #players do
            ...
            if player.name == "Monster" then
                playerUI.playerName.Scale(0.88, 1)
            end
            ...
            playerUI.atkZone.press.x = Misc.cameraX + 46
            playerUI.atkZone.press.y = Misc.cameraY + 78 - 38 * (i - 1)
            ...
            playerUI.atkZone.target.color = player.atkHitColor
            ...
            for i=1, 3 do
                ...
                t.SetPivot(0, 0.5)
                t.x = 270
                t.absy = playerUI.atkZone.press.absy + 19
                t.alpha = 0

                table.insert(playerUI.atkZone.visor["trails"], t)
            end
            playerUI.atkZone.visor["activeTrail"] = false

            self.UpdatePlayerHP(player)

            table.insert(self.uiStats, playerUI)
        end

        self.ManageUI()

        -- Put the UI out of the screen
        self.lowerUIRoot.absy = Misc.cameraY + -155
        self.upperUIRoot.absy = Misc.cameraY + -155
    end

    function self.DestroyUsedUI()
        for i = 1, #self.uiStats do
            local playerUI = self.uiStats[i]
            playerUI.shown = false    -- Are the Player's commands shown?
            playerUI.anim = nil       -- Anim of the UI (show or hide)
            playerUI.animTimer = 0    -- Anim timer

            -- Side bars of the buttons UI
            playerUI.sides.Remove()

            -- Sprite parent of all sprites used in the command UI's effect
            playerUI.sideEffectContainer.Remove()
            playerUI.sideEffects = { }

            -- Buttons names. Also used in a lot of places
            for i=1, #playerUI.buttons do
                playerUI.buttons[i].Remove()
                playerUI.buttons[i] = nil
            end

            -- Colorful outer of the stats UI
            playerUI.upperBg.Remove()

            -- Boring inner of the stats UI
            playerUI.upperBgStats.Remove()

            -- Life bar!
            -- TODO: Find out why the lifebar isn't at the same place when CYF is scaled up
            playerUI.lifebar.Remove()

            -- FaceSprite, ooo
            playerUI.faceSprite.Remove()

            -- Player's hp text
            playerUI.hpText.DestroyText()

            -- Player's maxhp text
            playerUI.maxHpText.DestroyText()

            -- Player's name
            playerUI.playerName.DestroyText()

            playerUI.atkZone.press.Remove()
            if i > 1 then
                playerUI.atkZone.separator.Remove()
            end

            playerUI.atkZone.bar.Remove()
            playerUI.atkZone.target.Remove()
            playerUI.atkZone.visor.Remove()
            
            for t=1, #playerUI.atkZone.visor["trails"] do
                playerUI.atkZone.visor["trails"][t].Remove()
            end
            playerUI.atkZone.visor["trails"] = { }
    ...
    function self.ManageUI()
        for i = 1, #CYK.allPlayers do
            ...
            player.UI.sides.absx = Misc.cameraX + xShift
            player.UI.upperBg.absx = Misc.cameraX + xShift

            local yFightUI = index and Misc.cameraY + 78 - 38 * (index - 1) or -500
    ...
    function self.UpdateUI(player)
        ...
        uiObject.atkZone.target.color = player.atkHitColor
    end
    ...
    function self.UpdatePlayerHurtFacesprite()
        for i=1, #CYK.allPlayers do
            local player = CYK.allPlayers[i]
            
            if player.UI.faceHurtTime > 1 then 
                player.UI.faceHurtTime = player.UI.faceHurtTime - 1
            elseif player.UI.faceHurtTime == 1 then 
    ...
    function self.Update()
        local isInBattle = CYK.state ~= "NONE"
        if isInBattle then
            for i = 1, #players do
            ...
            self.UpdatePlayerHurtFacesprite()
        end
    ...
    function self.CreateValueChangeText(value, entity, color, isMercy)
        ...
        local parent = CreateSprite("empty", "Bullet")
        parent.SetPivot(0, 0)
        parent.absx = container.x
        parent.absy = Misc.cameraY + container.y
    ...
    function self.UpdateHPChangeTexts()
        for i = #self.HPChangeTexts, 1, -1 do
            ...
            elseif frame <= 120 and (HPChangeText.yBounce ~= -16 or HPChangeText.yAccel ~= 0) then
                ...
                HPChangeText.parent.absy = Misc.cameraY + HPChangeText.y + HPChangeText.yBounce
            end
