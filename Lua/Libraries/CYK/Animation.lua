return function(self)
        ...
        for k, v in pairs(self.anims) do
            ...
            for k2, v2 in pairs(v) do
                ...
                for i = 1, #v2[1] do
                    ...
                    -- If an animation starts with act_, it'll look for it inside the Acts folder as a shorthand.
                    -- ...i don't encourage you to use that??? but i'll leave it there anyway?????
                    if string.find(k2, "act_") ~= nil then
                        k2 = k2:gsub("act_", "")
                        k2 = "Acts/" .. k2
                    end

                    --local file = "CreateYourKris/Players/Sam/Defend/0"
                    local file = "CreateYourKris/" .. (isPlayer and "Players/" or "Monsters/") .. k .. "/" .. k2 .. "/" .. cpath
                    v2[1][i] = file
                end
                -- Starts the anim if preloadAnimations is true to register it in CYK
                if preloadAnimations then
                    --if players_superhp[1] ~= nil then
                    --    error(v2[1][1])
                    --end
    ...
    function self.cleanAnims()
        self.anims = { }
        self.anims.followUps = { }
    end
    ...
    function self.StartArenaAnim(show, nowave, nostate)
        ...
        self.arenaAnimInfo.x = (show or nowave) and Misc.cameraX + arenapos[1] or arenainfo.x
        self.arenaAnimInfo.y = (show or nowave) and Misc.cameraY + arenapos[2] + arenasize[2] / 2 + 5 or arenainfo.y + arenainfo.height / 2 + 5
    ...
    function self.EndArenaAnim(nostate)
        ...
        for i = #self.arenaAnimEffects, 1, -1 do
            ...
            --if not (nostate == true) then error("no") end
            table.remove(self.arenaAnimEffects, i)
        end
    ...
    function self.StartSecondaryAnimation(type, target)
        local newAnimationRun = { name=type,
        timestart=Time.time, timeout=0,
        entity=target }
        
        if type == self.animationChannelsName.flashHeal then
            local secondaryFlash = CreateSprite("px", "Entity")
            secondaryFlash.SetParent(target.sprite["mask"])
            secondaryFlash.Scale(640, 480)
            secondaryFlash.MoveTo(0, 0)

            secondaryFlash.color = { 1, 1, 1 }
        ...
        elseif type == self.animationChannelsName.flashFailSpare then
            ...
            secondaryFlash.color = { .8, .8, 0 }
            secondaryFlash.alpha = 0

            newAnimationRun.f = secondaryFlash
            newAnimationRun.timeout = 0.76
        elseif type == self.animationChannelsName.flashFailPacify then
            local secondaryFlash = CreateSprite("px", "Entity")
            secondaryFlash.SetParent(target.sprite["mask"])
            secondaryFlash.Scale(640, 480)
            secondaryFlash.MoveTo(0, 0)

            secondaryFlash.color = { 0, 0, .8 }
            secondaryFlash.alpha = 0
        ...
        if newAnimationRun.timeout == 0 then
            -- Timeout value was never overwritten from its initial value, 0.
            error("The specified animation of type \"" .. tostring(type) .. "\" kept the newAnimationRun.timeout value as 0. Make sure to set the animation's endpoint value to at least higher than \"0.005\".")
        end

        table.insert(self.animationChannels, newAnimationRun)
    end

    -- All secondary animations are run isolated from here.
    function self.RunSecondaryAnimations()
        for i = #self.animationChannels, 1, -1 do
            local animation = self.animationChannels[i]

            local currentTime = Time.time - animation.timestart
            ...
            elseif animation.name == self.animationChannelsName.flashFailSpare then
                ...
                if timePercent < 40 then
                    animation.f.alpha = 1 * timePercent/40.0
                elseif timePercent > 60 then
                    animation.f.alpha = 1 - ( (timePercent-60)/40.0 )
                else
                    animation.f.alpha = 1
                end
            ...
            elseif animation.name == self.animationChannelsName.flashFailPacify then
                timePercent = (Input.Menu == 2) and timePercent*2 or timePercent
                
                if timePercent < 40 then
                    animation.f.alpha = 1 * timePercent/40.0
                elseif timePercent > 80 then
                    animation.f.alpha = 1 - ( (timePercent-80)/20.0 )
                else
                    animation.f.alpha = 1
                end
            ...
            if timePercent >= 100 then
                table.remove(self.animationChannels, i)  end
