...
return function(self)
    ...
    function self.CheckEntityFile(entity, entityName, isPlayer)
        ...
        if isPlayer then
            ...
            local colors = { "playerColor", "atkBarColor", "atkHitColor", "damageColor" }
            for h = 1, #colors do
    ...
    function self.SetEntitiesStart()
        -- Players table
        ...
        for i = 1, #self.allPlayers + #self.allEnemies do
            ...
            if entity.name == nil then
                ...
                if string.find(entity.name, "fall_") ~= nil then
                    entity.name = entity.name:gsub("fall_", "")
                end
            end

            -- Add the action frame in for the animation
            for k, v in pairs(entity.animations) do
                if v[3].actionFrame == nil then
                    v[3].actionFrame = #v[1]
                end
            end
            ...
            entity.posX = Misc.cameraX + (isPlayer and entityPos[1] or 640)
            entity.posY = Misc.cameraY + entityPos[2]
        ...
        for i = 1, #self.allPlayers do
            ...
            -- Set Player's HP to the one carried over by the Overworld
            if (i <= #Overworld.party) then
                self.allPlayers[i].hp = Overworld.party[i].hp
                self.allPlayers[i].maxhp = Overworld.party[i].maxhp
            end
        end
    ...
    function self.CleanEntites()

        for i = 1, #self.allPlayers do
            self.allPlayers[i].sprite.Remove()
            self.allPlayers[i] = nil
        end

        for i = 1, #self.allEnemies do
            self.allEnemies[i].sprite.Remove()
    ...
    function self.GetEntity(data, i, isPlayer)
        ...
        if isPlayer then
            ...
            entity.targetCursor["pointer"] = CreateSprite("CreateYourKris/UI/TargetCursor/2", "Entity")
            entity.targetCursor["pointer"].setParent(entity.targetCursor)
            entity.targetCursor["pointer"].x = 0
            entity.targetCursor["pointer"].y = 0
            entity.targetCursor["pointer"].SetPivot(0.5, 0.5)
            entity.targetCursor["pointer"].alpha = 0

            entity.targetCursor["pointer"]["speed"] = 1.8
            entity.targetCursor["pointer"]["angle"] = math.pi/2
    ...
    -- Commented, since it'll be called whenever an encounter *actually* starts.
        ...
        for i = 1, #pools do
            ...
            for j = 1, #pool do
                ...
                if entity.sprite["currAnim"] == "Hurt" then
                    -- Hurt animation for player
                    if entity.UI ~= nil then
                        local totalHurtAnimTime = entity.animations["Hurt"][2]
                        local currentTime = Time.time - entity.sprite["lastAnimTime"]
                        local timePercent = (currentTime / totalHurtAnimTime) * 100

                        if timePercent < 3 or (timePercent > 12 and timePercent <= 17) then
                            entity.sprite.absx = entity.posX - 10
                        elseif timePercent > 17 and timePercent < 25 then
                            entity.sprite.absx = entity.posX - 4
                        else
                            entity.sprite.absx = entity.posX
                        end
    ...
    -- Updates a target's flash sprite's to lighten. Also darkens the entities during the DEFENDING round.
    function self.UpdateEntityFlashMask()
        if self.state == "ENEMYSELECT" then
            -- Lighten the targeted entity.
        ...
        elseif self.Background and self.Background.anim then
            -- Darken all normally unharmed players
    ...
    function self.UpdateFlee(entity)
        ...
        if frame == 0 then
            ...
            entity.fleeSpritesNeeded = math.ceil(( Misc.cameraX + 640 - entity.sprite.absx) / 4)
            entity.fleeSpritesEnabled = 0
        end
        if frame < 30 then
